...
func TestFullUserFlow(t *testing.T) {
    ...
    regResp, err := http.Post(API_BASE_URL+"/users/register", "application/json", bytes.NewBuffer(regBody))
    ...
    loginResp, err := http.Post(API_BASE_URL+"/users/login", "application/json", bytes.NewBuffer(loginBody))
    ...
    extract := func(source map[string]interface{}, key string) interface{} {
        if val, ok := source[key]; ok { return val }
        if data, ok := source["data"].(map[string]interface{}); ok {
            if val, ok := data[key]; ok { return val }
        }
        return nil
    }

    if v, ok := extract(loginRes, "token").(string); ok { token = v }
    if v, ok := extract(loginRes, "User_ID").(float64); ok { userID = v }
    ...
    var chainID string
    if v, ok := extract(createRes, "chain_id").(string); ok { chainID = v }
    ...
    // 7. [边界测试] 重复注册
    dupRegResp, _ := http.Post(API_BASE_URL+"/users/register", "application/json", bytes.NewBuffer(regBody))
    if dupRegResp.StatusCode == 200 { t.Fatalf("边界测试失败: 允许重复注册") }
    t.Logf("边界测试通过: 禁止重复注册")

    // 8. [边界测试] 错误密码
    wrongLoginBody, _ := json.Marshal(map[string]string{"email": email, "password": "WrongPassword!"})
    wrongLoginResp, _ := http.Post(API_BASE_URL+"/users/login", "application/json", bytes.NewBuffer(wrongLoginBody))
    if wrongLoginResp.StatusCode == 200 { t.Fatalf("边界测试失败: 允许错误密码") }
    ...
    if noTokenResp.StatusCode == 200 { t.Fatalf("边界测试失败: 允许无Token") }
...
func TestMavlinkHandler(t *testing.T) {
    ...
    loginBody, _ := json.Marshal(map[string]string{"email": email, "password": password})
    loginResp, _ := http.Post(API_BASE_URL+"/users/login", "application/json", bytes.NewBuffer(loginBody))
    ...
    if v, ok := extract(handlerRes, "handler_id").(string); ok { handlerID = v }
    ...
    if handlerID != "" {
        ...
        if respGet.StatusCode == 200 { t.Logf("查询Handler成功") }
