...
func (m *MavlinkCommander) processMessage(msg message.Message, systemID, componentID uint8) {
	...
	case MAVLINK_MSG_ID_HEARTBEAT:
		...
		m.heartbeatStatus = heartbeat
		isArmed := (int(heartbeat.BaseMode) & int(common.MAV_MODE_FLAG_SAFETY_ARMED)) != 0
		log.Printf("[Mavlink] Heartbeat: SysID=%d, Type=%d, Armed=%v, SystemStatus=%d",
			systemID, heartbeat.Type, isArmed, heartbeat.SystemStatus)

	case MAVLINK_MSG_ID_SYS_STATUS:
		sysStatus := msg.(*common.MessageSysStatus)
		m.sysStatus = sysStatus
		log.Printf("[Mavlink] SysStatus: SysID=%d, Voltage=%dmV, Battery=%d%%, Load=%d%%",
			systemID, sysStatus.VoltageBattery, sysStatus.BatteryRemaining, sysStatus.Load/10)
	...
	case MAVLINK_MSG_ID_GLOBAL_POSITION_INT:
		...
		alt := float64(gp.RelativeAlt) / 1000.0
		log.Printf("[Mavlink] GlobalPosition: SysID=%d, Lat=%.7f, Lon=%.7f, Alt=%.1fm",
			systemID, lat, lon, alt)
	...
	case MAVLINK_MSG_ID_BATTERY_STATUS:
		...
		m.batteryStatus = bat
		log.Printf("[Mavlink] BatteryStatus: SysID=%d, Voltage=%dmV, Remaining=%d%%",
			systemID, bat.Voltages[0], bat.BatteryRemaining)

	case MAVLINK_MSG_ID_MISSION_ITEM_REACHED:
		mi := msg.(*common.MessageMissionItemReached)
		log.Printf("[Mavlink] MissionItemReached: SysID=%d, Seq=%d", systemID, mi.Seq)
...
func (m *MavlinkCommander) DoDigICamControl(sessionControl uint8, mode uint8, eZoomAbsolute uint16, zoomStep int8, focusLock uint8, shot uint8) error {
	return m.CommandLong(0, 0, uint16(common.MAV_CMD_DO_DIGICAM_CONTROL), 0,
...
func (m *MavlinkCommander) MissionWriteItem(seq uint16, current uint8, autocontinue uint8, frame uint8, command uint16, param1, param2, param3, param4, x, y, z float32) error {
	targetSys := m.GetTargetSystem()
...
func (m *MavlinkCommander) SendRCChannelsOverride(targetSystem, targetComponent uint8, chan1Raw, chan2Raw, chan3Raw, chan4Raw, chan5Raw, chan6Raw, chan7Raw, chan8Raw uint16) error {
...
func (m *MavlinkCommander) SetROI(lat, lon, alt float32) error {
	targetSys := m.GetTargetSystem()
	roiMsg := &common.MessageCommandLong{
		TargetSystem:    targetSys,
		TargetComponent: m.config.TargetComponent,
	...
	return m.node.WriteMessageAll(roiMsg)
}

func (m *MavlinkCommander) ConditionYaw(yaw, angleSpeed float32) error {
	targetSys := m.GetTargetSystem()
	...
	return m.node.WriteMessageAll(yawMsg)
}
...
const (
	...
	CMD_AUTO_RETURN         MavlinkCommandType = "AUTO_RETURN"
	CMD_START_RECORD        MavlinkCommandType = "START_RECORD"
	CMD_STOP_RECORD         MavlinkCommandType = "STOP_RECORD"
	CMD_ORBIT               MavlinkCommandType = "ORBIT"
	CMD_SET_RPM             MavlinkCommandType = "SET_RPM"
	CMD_SET_ROI             MavlinkCommandType = "SET_ROI"
)
...
func (m *MavlinkCommander) ExecuteCommand(cmd MavlinkCommand) error {
	...
	case CMD_AUTO_RETURN:
		return m.ReturnToLaunch()

	case CMD_START_RECORD:
		return m.DoDigICamControl(0, 0, 0, 0, 0, 1)

	case CMD_STOP_RECORD:
		return m.DoDigICamControl(0, 0, 0, 0, 0, 0)

	case CMD_ORBIT:
		return m.SetROI(cmd.Param5, cmd.Param6, cmd.Param7)

	case CMD_SET_RPM:
		return m.CommandLong(m.GetTargetSystem(), m.config.TargetComponent, 0, 0,
