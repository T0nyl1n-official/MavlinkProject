...
func SetRedisClients(clients *[]redis.Client) {
	redisClientsMu.Lock()
	defer redisClientsMu.Unlock()
	redisClients = clients
}

func GetRedisClientGlobal(dbIndex DBIndex) *redis.Client {
	redisClientsMu.RLock()
	defer redisClientsMu.RUnlock()
	return GetRedisClient(dbIndex, redisClients)
}
...
func GetRedisClient(dbIndex DBIndex, redisClients *[]redis.Client) *redis.Client {
	if redisClients == nil {
		return nil
	}
